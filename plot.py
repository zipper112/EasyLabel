...
def plotMultiRegion(img_path: str, region_list: list, color_list: list, line_width_list: list, save_path: str,
    ...
    :param place_list: 放大的区域放置在原图中什么位置列表 e.g.  
...
if __name__ == '__main__':
    ...
    for image_name in os.listdir(root_path):
        plotMultiRegion(os.path.join(root_path, image_name), region_list=region_list, line_width_list=line_list, color_list=color_list,
                    place_list=place_list, scale_list=scale_list, save_path=os.path.join(root_path, 'boxed_' + image_name), zoom_bool=zoom_bool)
